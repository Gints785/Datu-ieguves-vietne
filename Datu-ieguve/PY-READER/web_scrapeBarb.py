...
for base_url in base_urls:
    ...
    while True:
        ...
        for scraped_name, scraped_price in zip(scraped_product_names, scraped_product_prices):
            ...
            # Check if the scraped name exists in the Excel product names, and the product name is not empty or NaN
            for product_name, artikul in zip(product_names_in_db, artikuls_in_db):
                if not is_nan_or_empty(product_name):
                    ...
                    if product_name_cleaned in scraped_name_cleaned:
                        ...
                        logger.info(f'Product found in category "{title}" on page {page_number}: {product_name}, barbora_cena: {scraped_price}, artikuls: {artikul}')
...
found_products_df = pd.DataFrame({'barbora_nosaukums': found_product_names, 'barbora_cena': found_product_prices, 'artikuls': found_product_artikuls, 'barbora_datums': [today_date_str] * len(found_product_names), 'barbora_datums_7': [today_date_str] * len(found_product_names) })
        ...
        if existing_data:
            ...
            if existing_price != new_price:
                ...
                update_main_table_query = f"""
                    UPDATE {table_name}
                    SET "barbora_cena" = %s, "barbora_datums" = %s, "barbora_datums_7" = %s
                    WHERE "artikuls" = CAST(%s AS text)
                """
                cursor.execute(update_main_table_query, (values[found_products_df.columns.get_loc('barbora_cena')], values[found_products_df.columns.get_loc('barbora_datums')],values[found_products_df.columns.get_loc('barbora_datums_7')], values[found_products_df.columns.get_loc('artikuls')]))

                # Insert old row into history table
                insert_history_query = f"""
                    INSERT INTO {history_table_name} ("artikuls", "barbora_nosaukums", "barbora_cena", "barbora_datums")
                    VALUES (%s, %s, %s, %s)
                """
                cursor.execute(insert_history_query, (existing_data[0], existing_data[1], existing_data[2], existing_data[3]))
                ...
                update_date_query = f"""
                    UPDATE {table_name}
                    SET "barbora_datums" = %s
                    WHERE "artikuls" = CAST(%s AS text)
                """
                cursor.execute(update_date_query, (values[found_products_df.columns.get_loc('barbora_datums')], values[found_products_df.columns.get_loc('artikuls')]))
            ...
            """
            try:
                cursor.execute(insert_history_query, (values[found_products_df.columns.get_loc('artikuls')], values[found_products_df.columns.get_loc('barbora_nosaukums')], values[found_products_df.columns.get_loc('barbora_cena')], values[found_products_df.columns.get_loc('barbora_datums')]))
