    ...
    # Convert date to string format before insertion into the database
        ...
        if existing_data:
            ...
            if existing_price != new_price:
                ...
                update_date_query = f"""
                    UPDATE {table_name}
                    SET "citro_nosaukums" = %s, "citro_datums" = %s, citro_akcija = %s, "citro_url" = %s
                    WHERE "artikuls" = CAST(%s AS text)
                """
                cursor.execute(update_date_query, (values[found_products_df.columns.get_loc('citro_nosaukums')], values[found_products_df.columns.get_loc('citro_datums')], values[found_products_df.columns.get_loc('citro_akcija')], values[found_products_df.columns.get_loc('citro_url')], values[found_products_df.columns.get_loc('artikuls')]))
